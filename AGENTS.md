...
## Testing Guidelines
Unit and CLI tests run through `qmk pytest`, which wraps `nose2` against the modules in `lib/python`. Target failing scenarios by mirroring the module path, e.g., `tests/lib/python/qmk/cli/test_new_command.py`. For firmware, build the affected keyboard/keymap combinations locally and attach compile logs when CI cannot exercise custom hardware. Use `qmk lint` early to catch metadata regressions, and add docs or sample keymaps under `tests/` when introducing new features.
