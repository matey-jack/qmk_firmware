...
#include "logical_layer_lights.c"
...
enum custom_keycodes {
    ...
    MX_LLL,  // toggle logical layer lights mode
...
#define RGB_NIGHTLIGHT_BASE 0xF2, 0xDE, 0xA2
#define RGB_NIGHTLIGHT_TOGGLES 0xF2, 0xC0, 0xA2
    ...
    if (layer == 0) {
        for (uint8_t row = 0; row < MATRIX_ROWS; ++row) {
            for (uint8_t col = 0; col < MATRIX_COLS; ++col) {
                ...
                if (index >= led_min && index < led_max && index != NO_LED) {
                    ...
                    if (get_type(keycode) == KEY_COLOR_LAYER_TOGGLE) {
                        rgb_matrix_set_color(index, RGB_NIGHTLIGHT_TOGGLES);
                    } else {
                        rgb_matrix_set_color(index, RGB_NIGHTLIGHT_BASE);
                    }
                }
            }
        }
        ...
        for (uint8_t row = 0; row < MATRIX_ROWS; ++row) {
            for (uint8_t col = 0; col < MATRIX_COLS; ++col) {
                ...
                if (index >= led_min && index < led_max && index != NO_LED) {
                    uint16_t keycode = keymap_key_to_keycode(layer, (keypos_t){col,row});
                    set_color_for_keycode(index, keycode);
                }
            }
        }
    }
