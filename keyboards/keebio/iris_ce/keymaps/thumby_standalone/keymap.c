...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [L_BASE] = LAYOUT(
            ...
            KC_TAB , KC_Q, KC_W, KC_D, KC_F, KC_B   ,                     KC_Z   , KC_K, KC_U   , KC_O  , KC_P   , MX_QUOT,
            KL_SHFT, KC_A, KC_S, KC_R, KC_T, KC_G   ,                     US_DIAE, KC_N, KC_I   , KC_L  , KC_H   , KL_SHFT,
            KC_LCTL, L2_Y, L2_X, KC_C, KC_V, KC_SLSH, KC_LGUI,   KC_RGUI , KC_J   , KC_M, KC_COMM, KC_DOT, L2_MINS, L3_INS,
    ...
    S(KC_TAB ), S(KC_Q), S(KC_W), S(KC_D), S(KC_F), S(KC_B   ),                         S(KC_Z   ), S(KC_K), S(KC_U), S(KC_O), S(KC_P),   MX_DQUO ,
      KC_LSFT , S(KC_A), S(KC_S), S(KC_R), S(KC_T), S(KC_G   ),                           KC_PLUS , S(KC_N), S(KC_I), S(KC_L), S(KC_H),   KC_RSFT ,
    S(KC_LCTL), S(KC_Y), S(KC_X), S(KC_C), S(KC_V),   US_EURO , S(KC_LGUI), S(KC_RGUI), S(KC_J   ), S(KC_M), KC_SCLN, KC_COLN, KC_UNDS,   L3_INS,
    ...
    [L_ALTGR] = LAYOUT(
			...
			KC_LCTL, KC_TRNS, KC_TOP , KC_PGUP, KC_PGDN, KC_BOTT, KC_LGUI,     KC_RGUI, US_MUL , KC_EQL , KC_LT  , KC_GT  , MX_DASH, KC_NO  ,
    ...
    switch (keycode) {
        ...
        case MX_VERS:
            if (record->event.pressed) {
                send_string_with_delay("Thumby mapping, beta-01-rs, ", SEND_STRING_DELAY_MS);
